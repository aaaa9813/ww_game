...
void CServer::OnTimeMatch() {
	...
	int num = 0;
	if (m_Matchlist.size() >= gameusernum) {
...
//		ds.Write((unsigned int) gameusernum);			//write size

		for (int i = 0; i < gameusernum; i++) {
...
//			ds.Write((unsigned int) index);
			...
			data.userindex[i].uid = tmp[i]->GetId();
			data.userindex[i].index = index;
